...
def init_state():
    if "pipeline" not in st.session_state:
        st.session_state["pipeline"] = load_pipeline(MODEL_PATH)
        st.session_state["pipeline2"] = load_pipeline(MODEL_PATH2)
...
def page1():
    ...
    with tab1:
        ...
        st.markdown(
            """<hr style="height:10px;border:none;color:#333;background-color:#333;" /> """, unsafe_allow_html=True
        )
        ...
        with col1:
            option = st.radio("Please enter a movie or select from the list", ("Enter Movie Name", "Select from list"))
                    ...
                    if st.button("Get Revenue"):
                        if df is None and path is None:
                            ...
                            with st.spinner("Wait for it...Fetching model parameters"):
                                ...
                                output_results = {
                                    "Budget(US Dollar)": millify(df["budget"][0]),
        ...
        with col3:
            ...
            if prediction is not None:
                ...
                chart = (
                    alt.Chart(source)
                    .mark_bar()
                    .encode(x="Revenue", y="US Dollar", color=alt.Color("Legend", legend=None))
                )
    ...
    with tab2:
        ...
        with col3:
            belongs_to_collection = st.selectbox("Do you want to make a sequel?", ("Yes", "No"))
        ...
        prediction = predict(st.session_state["pipeline2"], features)
        col1, col2, col3 = st.columns(3)
        with col2:
            st.header("")
